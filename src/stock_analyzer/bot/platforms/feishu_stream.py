...
from stock_analyzer.bot.models import BotMessage, BotResponse, ChatType
    ...
    def _send_interactive_card(
        ...
        try:
            ...
            if message_id:
                ...
                response = self._client.im.v1.message.reply(request)  # type: ignore[union-attr]
            else:
                # 主动发送消息
                if chat_id is None:
                    logger.error("[Feishu Stream] 发送消息失败: chat_id 不能为空")
                    return False
                request = (
                ...
                response = self._client.im.v1.message.create(request)  # type: ignore[union-attr]
    ...
    def send_to_chat(self, chat_id: str, text: str, receive_id_type: str = "chat_id") -> bool:
        ...
        if content_bytes > self._max_bytes:
            ...
            return chunk_feishu_content(
                formatted_text,
                self._max_bytes,
                lambda chunk: self._send_interactive_card(chunk, chat_id=chat_id, receive_id_type=receive_id_type),
            )
...
class FeishuStreamHandler:
    ...
    def _parse_event_message(self, event: P2ImMessageReceiveV1) -> BotMessage | None:
        ...
        try:
            ...
            # 提取命令（去除 @机器人）
            mentions_list: list = list(message_data.mentions) if message_data.mentions else []
            content = self._extract_command(raw_content, mentions_list)
...
class FeishuStreamClient:
    ...
    def _create_message_handler(self) -> Callable[[BotMessage], BotResponse]:
        ...
        def handle_message(message: BotMessage) -> BotResponse:
            from stock_analyzer.bot.dispatcher import get_dispatcher
    ...
    def _create_event_handler(self) -> lark.EventDispatcherHandler:
        """创建事件分发处理器"""
        # 确保 app_id 和 app_secret 不为 None（已在 __init__ 中验证）
