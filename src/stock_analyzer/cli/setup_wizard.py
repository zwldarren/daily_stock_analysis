...
def _configure_single_channel(channel_type: str) -> dict[str, str]:
    ...
    if channel_type == "wechat":
        urls: list[str] = []
        while True:
            url = questionary.text("请输入微信企业机器人 Webhook URL:").ask()
            if url:
                urls.append(url)
                if not questionary.confirm("是否添加更多微信机器人?", default=False).ask():
                    break
            else:
                break
        if urls:
            config["WECHAT_WEBHOOK_URLS"] = ",".join(urls)

    elif channel_type == "feishu":
        urls = []
        while True:
            url = questionary.text("请输入飞书机器人 Webhook URL:").ask()
            if url:
                urls.append(url)
                if not questionary.confirm("是否添加更多飞书机器人?", default=False).ask():
                    break
            else:
                break
        if urls:
            config["FEISHU_WEBHOOK_URLS"] = ",".join(urls)
        ...
        while True:
            ...
            if token and chat_id:
                ...
                if not questionary.confirm("是否添加更多 Telegram Bot?", default=False).ask():
                    break
            else:
                break
        if tokens:
            config["TELEGRAM_BOT_TOKENS"] = ",".join(tokens)
            config["TELEGRAM_CHAT_IDS"] = ",".join(chat_ids)
        ...
        console.print("\n[dim]Discord 支持两种方式:[/dim]")
        console.print("[dim]1. Webhook 模式 (推荐，配置简单)[/dim]")
        console.print("[dim]2. Bot API 模式 (需要 Bot Token 和频道 ID)[/dim]")
        discord_mode = questionary.select(
            "请选择 Discord 配置方式:",
            choices=["Webhook 模式", "Bot API 模式"],
            default="Webhook 模式",
        ).ask()
        if discord_mode == "Webhook 模式":
            urls = []
            while True:
                url = questionary.text("请输入 Discord Webhook URL:").ask()
                if url:
                    urls.append(url)
                    if not questionary.confirm("是否添加更多 Discord Webhook?", default=False).ask():
                        break
                else:
                    break
            if urls:
                config["DISCORD_WEBHOOK_URLS"] = ",".join(urls)
        else:
            tokens: list[str] = []
            channel_ids: list[str] = []
            while True:
                token = questionary.password("请输入 Discord Bot Token:").ask()
                channel_id = questionary.text("请输入 Discord 频道 ID:").ask()
                if token and channel_id:
                    tokens.append(token)
                    channel_ids.append(channel_id)
                    if not questionary.confirm("是否添加更多 Discord Bot?", default=False).ask():
                        break
                else:
                    break
            if tokens:
                config["DISCORD_BOT_TOKENS"] = ",".join(tokens)
                config["DISCORD_CHANNEL_IDS"] = ",".join(channel_ids)

    elif channel_type == "pushover":
        user_keys: list[str] = []
        api_tokens: list[str] = []
        while True:
            user_key = questionary.text("请输入 Pushover User Key:").ask()
            api_token = questionary.text("请输入 Pushover API Token:").ask()
            if user_key and api_token:
                user_keys.append(user_key)
                api_tokens.append(api_token)
                if not questionary.confirm("是否添加更多 Pushover 配置?", default=False).ask():
                    break
            else:
                break
        if user_keys:
            config["PUSHOVER_USER_KEYS"] = ",".join(user_keys)
            config["PUSHOVER_API_TOKENS"] = ",".join(api_tokens)

    elif channel_type == "pushplus":
        tokens = []
        while True:
            token = questionary.text("请输入 PushPlus Token:").ask()
            if token:
                tokens.append(token)
                if not questionary.confirm("是否添加更多 PushPlus Token?", default=False).ask():
                    break
            else:
                break
        if tokens:
            config["PUSHPLUS_TOKENS"] = ",".join(tokens)

    elif channel_type == "serverchan3":
        keys = []
        while True:
            key = questionary.text("请输入 Server酱3 SendKey:").ask()
            if key:
                keys.append(key)
                if not questionary.confirm("是否添加更多 Server酱3 SendKey?", default=False).ask():
                    break
            else:
                break
        if keys:
            config["SERVERCHAN3_SENDKEYS"] = ",".join(keys)
...
def ask_advanced_options() -> dict[str, Any]:
    ...
    if enable_schedule:
        ...
        market_review = questionary.confirm("是否启用大盘复盘?", default=True).ask()
