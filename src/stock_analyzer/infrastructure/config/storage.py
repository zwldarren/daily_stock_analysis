...
This module provides database-backed configuration storage,
implementing the IConfigStorage interface from the config layer.
This follows DDD principles by keeping infrastructure details
separate from domain configuration logic.
"""
...
class ConfigStorageImpl(IConfigStorage):
    """Database-backed configuration storage implementation.

    This class implements the IConfigStorage interface using SQLAlchemy
    ...
    def save_to_db(self, config_dict: dict[str, Any]) -> None:
        ...
        if not self.db_session:
            raise RuntimeError("Database not initialized, please provide db_url")
...
def load_merged_config_with_db(db_url: str | None = None) -> dict[str, str]:
    """Load merged configuration from multiple sources.

    Priority: Environment variables > .env file > Database
    ...
    if db_url:
        ...
        try:
            storage = ConfigStorageImpl(db_url)
            db_config = storage.load_from_db()
        except Exception:
            pass  # Ignore database loading failures
        finally:
            if storage:
                storage.close()

    # 2. Load from .env file (medium priority)
    from stock_analyzer.config.storage import load_merged_config

    env_config = load_merged_config()

    # 3. Merge: Database < .env file < Environment variables
