...
class TushareFetcher(BaseFetcher):
    ...
    def get_realtime_quote(self, stock_code: str, **kwargs) -> UnifiedRealtimeQuote | None:
        ...
        try:
            ts_code = self._convert_stock_code(stock_code)
            # 尝试调用 Pro 实时接口 (需要积分)
            df = self._api.quotation(ts_code=ts_code)  # type: ignore[union-attr]
    ...
    def get_main_indices(self) -> list[dict] | None:
        ...
        try:
            ...
            for ts_code, name in indices_map.items():
                try:
                    df = self._api.index_daily(ts_code=ts_code, start_date=start_date, end_date=end_date)  # type: ignore[union-attr]
                    if df is not None and not df.empty:
                        row = df.iloc[0]  # 最新一天
    ...
    def get_market_stats(self) -> dict | None:
        ...
        try:
            ...
            trade_cal = api.trade_cal(  # type: ignore[union-attr]
                exchange="",
            ...
            df = api.daily(trade_date=last_date)  # type: ignore[union-attr]
