...
from .formatters import format_feishu_markdown
...
class NotificationService:
    ...
    def generate_wechat_summary(self, results: list[AnalysisResult]) -> str:
        ...
        lines = [
            ...
            "",
            f"> 共 **{len(results)}** 只 | 🟢买入:{buy_count} "
            f"🟡持有:{hold_count} 🔴卖出:{sell_count} | 均分:{avg_score:.0f}",
            "",
        ]
    ...
    def send_to_email(self, content: str, subject: str | None = None) -> bool:
        ...
        try:
            ...
            msg["From"] = sender  # type: ignore[index]
            ...
            # 自动识别 SMTP 配置
            domain = str(sender).split("@")[-1].lower()
    ...
    def send_to_telegram(self, content: str) -> bool:
        ...
        if not bot_token or not chat_id:
            logger.error("Telegram 配置不完整，发送失败")
            return False
    ...
    def _send_telegram_message(
        self, api_url: str, chat_id: str, text: str, message_thread_id: str | None = None
    ) -> bool:
        """发送单条 Telegram 消息"""
    ...
    def _send_telegram_chunked(
        self, api_url: str, chat_id: str, content: str, max_length: int, message_thread_id: str | None = None
    ) -> bool:
        """分段发送长 Telegram 消息"""
        # 按段落分割
        sections = content.split("\n---\n")
    ...
    def send_to_pushover(self, content: str, title: str | None = None) -> bool:
        ...
        if not user_key or not api_token:
            logger.error("Pushover 配置不完整，发送失败")
            return False
    ...
    def _send_pushover_chunked(
        ...
        for section in sections:
            ...
            new_length = current_length + len(separator) + len(section) if current_chunk else len(section)
    ...
    def _send_dingtalk_chunked(self, url: str, content: str, max_bytes: int = 20000) -> bool:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if body_bytes > max_bytes:
                ...
                payload["markdown"]["text"] = self._truncate_to_bytes(payload["markdown"]["text"], hard_budget)  # type: ignore[index]
    ...
    def _send_feishu_stream_reply(self, chat_id: str, content: str) -> bool:
        ...
        """
        try:
            from stock_analyzer.bot.platforms.feishu_stream import (
    ...
    def send_to_pushplus(self, content: str, title: str | None = None) -> bool:
        """
        推送消息到 PushPlus
    ...
    def _process_mermaid_charts(self, content: str) -> str:
        ...
        for paragraph in paragraphs:
            ...
            if re.match(
                r"\b(?:graph|flowchart|sequenceDiagram|classDiagram|stateDiagram|erDiagram|gantt|pie)\b",
                paragraph.strip(),
            ):
    ...
    def _create_discord_embed(self, content: str) -> dict[str, Any]:
        ...
        description = "\n".join([line for line in description.split("\n") if line.strip()])[
            :2048
        ]  # Discord description limit is 2048 characters
        ...
        embed = {
            ...
            "fields": fields,
            "author": {"name": "A股智能分析系统", "icon_url": "https://picsum.photos/200"},
            "thumbnail": {
